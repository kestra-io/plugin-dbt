...
RUN pip install --no-cache-dir kestra dbt-bigquery dbt-snowflake dbt-redshift dbt-postgres dbt-spark dbt-databricks dbt-dremio dbt-trino dbt-duckdb dbt-fabric dbt-synapse